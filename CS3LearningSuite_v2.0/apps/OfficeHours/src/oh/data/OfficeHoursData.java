...
public class OfficeHoursData implements AppDataComponent {
    ...
    // CHECKS IF OFFICE HOUSRS ARE SELECTED
    public boolean isOHSelected() {
        AppGUIModule gui = app.getGUIModule();
        TableView ohTable = (TableView)gui.getGUINode(OH_OFFICE_HOURS_TABLE_VIEW);
        return ohTable.getSelectionModel().getSelectedCells() != null;
    }
    
    public int getSelectedColumn() {
        AppGUIModule gui = app.getGUIModule();
        TableView ohTable = (TableView)gui.getGUINode(OH_OFFICE_HOURS_TABLE_VIEW);
        TablePosition pos = (TablePosition)ohTable.getSelectionModel().getSelectedCells().get(0);
    ...
    public TimeSlot getSelectedSlot() {
        AppGUIModule gui = app.getGUIModule();
        TableView ohTable = (TableView)gui.getGUINode(OH_OFFICE_HOURS_TABLE_VIEW);
        return (TimeSlot)ohTable.getSelectionModel().getSelectedItem();
    }
